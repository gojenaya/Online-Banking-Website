...
const calcDisplayBalance = function(accs){
  ...
  labelBalance.textContent = formatCurrency(accs.balance, accs.locale, accs.currency);
};

const calcDisplaySummary = function(accs){
  //In summary
  const income = accs.movements.filter(mov => mov > 0).reduce((acc, mov) => acc + mov, 0);
  labelSumIn.textContent = formatCurrency(income, accs.locale, accs.currency);
  //Out summary
  const out = accs.movements.filter(mov => mov < 0).reduce((acc, mov) => acc + mov, 0);
  labelSumOut.textContent = formatCurrency(Math.abs(out), accs.locale, accs.currency);

  //Interest
  const interest = accs.movements.filter(mov => mov > 0).map(deposit => deposit * accs.interestRate /100).filter((int, i ,arr) => {return int >= 1}).reduce((acc,int) => acc + int, 0 )
  labelSumInterest.textContent = formatCurrency(interest, accs.locale, accs.currency);
};
...
btnLoan.addEventListener('click', function(e){
  ...
  if(amount > 0 && currentAccount.movements.some(amt => amt >= 0.1 * amount) )
  {
    setTimeout(function(){
      currentAccount.movements.push(amount);
      currentAccount.movementsDates.push(new Date().toISOString());
